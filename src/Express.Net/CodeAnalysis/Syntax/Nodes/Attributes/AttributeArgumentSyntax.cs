namespace Express.Net.CodeAnalysis.Syntax.Nodes.Attributes;

public class AttributeArgumentSyntax : SyntaxNode
{
    public AttributeArgumentSyntax(SyntaxTree syntaxTree, SyntaxToken name, SyntaxToken expression)
        : base(syntaxTree)
    {
        Kind = SyntaxKind.AttributeArgument;
        Expression = expression;
        Name = name;
    }

    public AttributeArgumentSyntax(SyntaxTree syntaxTree, SyntaxToken expression)
        : base(syntaxTree)
    {
        Kind = SyntaxKind.AttributeArgument;
        Expression = expression;
    }

    public override SyntaxKind Kind { get; init; }

    public SyntaxToken? Name { get; init; }

    public SyntaxToken Expression { get; init; }

    public override IEnumerable<SyntaxNode> GetChildren()
    {
        if (Name is not null)
        {
            yield return Name;
        }

        yield return Expression;
    }
}
